...
class Scraper:
    def __init__(self, db_conn, db_lock, memory_lock, db_buffer, table_name="sextop1_videos", domain=None):
        self.db_conn = db_conn
        self.db_lock = db_lock
        self.memory_lock = memory_lock
        self.db_buffer = db_buffer
        self.table_name = table_name
        self.domain = domain if domain else "sextop1.buzz"
        self.session = curl_requests.Session(impersonate="chrome120")
        self.sync_lock = threading.Lock()
        self.referer = f"https://{self.domain}/"
        self.source_name = "Sextop1"
    ...
    def update_sync_tasks_from_menu(self):
        ...
        tasks = [
            ...
            f"https://{self.domain}/the-loai/jav-khong-che/page/{{page}}/",
            f"https://{self.domain}/the-loai/phim-sex-vietsub/page/{{page}}/",
            f"https://{self.domain}/the-loai/jav-hd/page/{{page}}/"
        ]
        with self.db_lock:
            cursor = self.db_conn.cursor()
            for url in tasks:
    ...
    def _sync_list_page(self, url_pattern, page):
        ...
        try:
            ...
            nav_links = soup.select('.pagination a.page-numbers')
            for a in nav_links:
                try:
            ...
            items = soup.select('article, .post-item, .item')
            now = int(time.time())
            for idx, item in enumerate(items):
                ...
                vid_id = href.strip('/').split('/')[-1]
                if not vid_id or len(vid_id) < 3: continue
                
                title_tag = item.select_one('.entry-title, .title, h2') or a_tag
    ...
    def get_video_url(self, vid_id, force_refresh=False):
        ...
        if not id_match:
            custom_log(self.source_name, "❌ Không phân tích được ID phim từ bài viết")
            return None
        ...
        with self.sync_lock:
            ...
            try:
                ...
                for a in soup.select('a[rel="tag"]'):
                    href = a.get('href', '')
                    text = a.text.strip()
                    if '/dien-vien/' in href or '/actor/' in href:
                        actress_arr.append(text)
                    elif '/the-loai/' in href or '/category/' in href:
                        genre_arr.append(text)
                    elif '/nha-san-xuat/' in href or '/studio/' in href:
                        maker = text
